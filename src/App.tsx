...
type AppTab = 'fold' | 'viewer';

export default function App() {
  const [tab, setTab]                   = useState<AppTab>('fold');
  ...
  /**
   * Load the 94 ADAPT/CCF crRNAs (direct repeat + spacer, RNA) into the
   * FASTA box. These are the crRNAs from the measured no-target background
   * set, so folding them here is directly comparable to that data.
   * Served statically from public/, so this works on Amplify with no backend.
   */
  const loadAdapt94 = async () => {
    setPresetLoading(true);
    setPresetError(null);
    try {
      const res = await fetch(`${import.meta.env.BASE_URL}adapt_94_crrnas.fasta`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const text = await res.text();
      const n = (text.match(/^>/gm) || []).length;
      if (n === 0) throw new Error('no FASTA records found');
      setFastaText(text);
      setFiles([]);
    } catch (err) {
      ...
      >
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-3">
            <div style={{ width: 32, height: 32, background: 'var(--accent-red)' }} />
            <span className="font-display text-lg" style={{ color: 'var(--text-primary)', letterSpacing: '0.5px' }}>
              RNA FOLD
            </span>
          </div>
          ...
          <div className="flex flex-col gap-1">
            {([
              { id: 'fold',   label: 'STRUCTURE PREDICTION' },
              { id: 'viewer', label: 'SEQUENCE VIEWER' },
            ] as { id: AppTab; label: string }[]).map(({ id, label }) => (
              <button
                key={id}
                onClick={() => setTab(id)}
                className="px-3 py-2 text-left font-display text-xs transition-all"
                style={{
                  borderLeft: tab === id ? '2px solid var(--accent-red)' : '2px solid transparent',
            ...
            <>
              ...
              <div className="flex flex-col gap-3">
                ...
                <button
                  ...
                  className="w-full px-3 py-2 text-xs text-left transition-opacity disabled:opacity-50"
                  style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text-primary)' }}
                  title="Loads all 94 ADAPT/CCF crRNAs (direct repeat + spacer) into the FASTA box"
                >
                  {presetLoading ? 'Loading…' : 'Load 94 ADAPT crRNAs'}
                </button>
                {presetError && (
                  <p className="text-xs" style={{ color: 'var(--accent-red)' }}>
                    Could not load preset: {presetError}
                  </p>
                )}
              </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input type="range" min="1" max="10" step="0.5" value={gamma}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input type="range" min="-5" max="10" step="1" value={bpWeightExp}
                    onChange={(e) => setBpWeightExp(parseInt(e.target.value))}
                    className="w-full" style={{ accentColor: 'var(--accent-red)' }}
                  />
                  <div className="flex justify-between text-xs" style={{ color: 'var(--text-muted)' }}>
                    <span>2^-5</span><span>2^10</span>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-col gap-2 max-h-[40vh] overflow-y-auto">
                    {results.map((r, i) => {
                      ...
                      return (
                        ...
                        >
                          <span className="text-xs truncate flex-1" title={r.seq_id}>{r.seq_id}</span>
                          {r.mfe !== null && (
                            <span className="font-display text-xs flex-shrink-0"
        ...
        {/* Footer */}
        <div className="flex flex-col gap-2">
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>ViennaRNA + CentroidFold</p>
        </div>
      </aside>

      {/* Main — Viewer tab takes full control of its own layout */}
      {tab === 'viewer' ? (
        ...
        <main className="flex-1 overflow-hidden flex flex-col" style={{ padding: '48px' }}>
          {/* Global error */}
          {error && (
            <div className="mb-6 px-4 py-3 text-sm flex-shrink-0"
          ...
          <div className="flex items-end justify-between mb-12 flex-shrink-0">
            <div className="flex flex-col gap-2">
              <h1 className="font-display"
                style={{ fontSize: '40px', fontWeight: '300', color: 'var(--text-primary)', letterSpacing: '-1px' }}
              >
            ...
            {hasResults && (
              <button onClick={() => downloadTSV(results)}
            ...
            <div className="flex gap-6 mb-12 flex-shrink-0">
              {[
                ...
                <div key={label} className="flex-1 flex flex-col gap-2"
                  style={{ border: '1px solid var(--border)', padding: '28px' }}
                >
                  <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{label}</span>
                  <span className="font-display"
                    style={{ fontSize: '36px', fontWeight: '100', color: 'var(--text-primary)', letterSpacing: '-1px' }}
                  >
                    {value}
                  </span>
                </div>
          ...
          {loading && !current && (
            <div className="flex-1 flex flex-col items-center justify-center gap-6">
              <div className="animate-pulse-slow"
                style={{ width: 80, height: 80, background: 'var(--surface)', border: '1px solid var(--border)',
          ...
          {!loading && !hasResults && !error && (
            <div className="flex flex-col items-center justify-center h-full gap-6">
              ...
              <div className="text-center">
                <p className="font-display text-2xl" style={{ color: 'var(--text-primary)', fontWeight: '300' }}>
                  No Results Yet
                </p>
